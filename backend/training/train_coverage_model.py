...
from .model_enhanced import CoverageMamba_Enhanced
...
class CoverageDataset(Dataset):
    def __init__(self, csv_path, normalize=True):
        ...
        # Coverage特定的列
        self.feature_cols = ['uav_count', 'rsu_count', 'vehicle_density', 'coverage_radius_m', 
                            'area_size_m2']
        self.target_cols = ['coverage_rate']
        ...
        if normalize:
            ...
            # Coverage rate在0-1之间，不需要normalize
        else:
            self.X_scaler = None
        
        print(f"✅ 加载Coverage数据: {len(self)} 样本")
...
def train_model(csv_path, epochs=120, batch_size=128, lr=0.0003):
    device = torch.device("mps" if torch.backends.mps.is_available() else "cpu")
    print(f"\n{'='*80}\n🚀 Coverage增强模型训练\n{'='*80}\n设备: {device}, Epochs: {epochs}\n")
    
    dataset = CoverageDataset(csv_path)
    train_size = int(0.85 * len(dataset))
    train_ds, val_ds = random_split(dataset, [train_size, len(dataset) - train_size])
    train_loader = DataLoader(train_ds, batch_size=batch_size, shuffle=True)
    val_loader = DataLoader(val_ds, batch_size=batch_size, shuffle=False)
    
    model = CoverageMamba_Enhanced(input_dim=5, hidden_dim=128, output_dim=1).to(device)
    ...
    best_r2, best_epoch, patience, patience_counter = -float('inf'), 0, 30, 0
    ...
    for epoch in range(epochs):
        ...
        print(f"   R²: {current_r2:.6f}, MAE: {mae:.6f}")
        print(f"   Coverage: True avg={all_targets.mean():.4f}, Pred avg={all_preds.mean():.4f}")
        
        if current_r2 > best_r2:
            best_r2, best_epoch, patience_counter = current_r2, epoch+1, 0
            os.makedirs("ai_models/coverage/checkpoints", exist_ok=True)
            torch.save({
                'model_state_dict': model.state_dict(),
                'r2': best_r2,
                'epoch': best_epoch
            }, "ai_models/coverage/checkpoints/coverage_enhanced_best.pth")
...
if __name__ == "__main__":
    ...
    # 使用新的多样化数据
    train_model(os.path.join(BASE, "data/training/coverage/coverage_train_12k_diverse.csv"))
