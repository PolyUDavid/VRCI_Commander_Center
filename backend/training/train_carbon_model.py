...
from .model_enhanced import CarbonLightTS_Enhanced
...
class CarbonDataset(Dataset):
    def __init__(self, csv_path, normalize=True):
        ...
        # Carbon特定的列 - 重新组织为10年序列
        self.feature_cols = ['annual_energy_savings_kwh', 'embodied_carbon_tonnes', 
                            'carbon_intensity_kg_per_kwh']
        
        # 按sample_id分组，每个sample有10年数据
        self.X = []
        self.y = []
        for sample_id in self.df['sample_id'].unique():
            sample_df = self.df[self.df['sample_id'] == sample_id].sort_values('year')
            if len(sample_df) == 10:  # 确保有完整10年
                # 使用第一年的特征
                features = sample_df[self.feature_cols].iloc[0].values
                # 所有10年的净累计碳作为目标
                targets = sample_df['net_cumulative_tonnes'].values
        ...
        print(f"✅ 加载Carbon数据: {len(self)} 样本")
...
def train_model(csv_path, epochs=200, batch_size=128, lr=0.0003):
    device = torch.device("mps" if torch.backends.mps.is_available() else "cpu")
    print(f"\n{'='*80}\n🚀 Carbon增强模型训练\n{'='*80}\n设备: {device}, Epochs: {epochs}\n")
    
    dataset = CarbonDataset(csv_path)
    train_size = int(0.85 * len(dataset))
    train_ds, val_ds = random_split(dataset, [train_size, len(dataset) - train_size])
    train_loader = DataLoader(train_ds, batch_size=batch_size, shuffle=True)
    val_loader = DataLoader(val_ds, batch_size=batch_size, shuffle=False)
    
    model = CarbonLightTS_Enhanced(input_dim=3, hidden_dim=128, output_years=10).to(device)
    ...
    best_r2, best_epoch, patience, patience_counter = -float('inf'), 0, 40, 0
    ...
    for epoch in range(epochs):
        ...
        if current_r2 > best_r2:
            ...
            os.makedirs("ai_models/carbon/checkpoints", exist_ok=True)
            torch.save({
                'model_state_dict': model.state_dict(),
                'r2': best_r2,
                'epoch': best_epoch
            }, "ai_models/carbon/checkpoints/carbon_enhanced_best.pth")
...
if __name__ == "__main__":
    ...
    train_model(os.path.join(BASE, "data/training/carbon/carbon_train_5k_10y.csv"))
